                                        ...
                                        my_matrix_t *unselected, uint32_t i_selected)
{
    for (uint32_t i = 0; i < evo->pop_size - i_selected; ++i) {
        ...
        double *arr;
        uint32_t arr_size = evo->get_agent_array(cell_parent, &arr, evo->params);
        ...
        evo->agent_from_array(cell_child, arr, evo->params);
        free(arr);
        evo->reset_agent(cell_parent, evo->pop, evo->pop_size, evo->params);
        evo->reset_agent(cell_child, evo->pop, evo->pop_size, evo->params);
    }
}
